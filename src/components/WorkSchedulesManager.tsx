  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 8, background: "var(--neutral-50)", border: "1px solid var(--border-default)", borderRadius: 10, padding: "8px 12px", marginBottom: 14 }}>
        ...
        <input placeholder="Pesquisar..." value={search} onChange={e => setSearch(e.target.value)} style={{ border: "none", outline: "none", background: "transparent", fontSize: 13, color: "var(--text-heading)", flex: 1 }} />
        <span style={{ fontSize: 12, color: "var(--text-muted)" }}>{filtered.length} resultado{filtered.length === 1 ? "" : "s"}</span>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1.4fr 1.2fr 0.9fr 64px", padding: "6px 4px", borderBottom: "1px solid var(--border-default)", marginBottom: 4 }}>
      ...
      {loading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "48px 0", gap: 8 }}>
          <Clock size={32} color="var(--neutral-300)" />
          <p style={{ fontSize: 13, color: "var(--text-muted)", margin: 0 }}>{items.length === 0 ? "Nenhum horário criado" : "Nenhum resultado"}</p>
          {items.length === 0 && <p style={{ fontSize: 12, color: "var(--text-muted)", margin: 0 }}>Clique em "Criar" para adicionar um horário</p>}
        </div>
      ) : (
        filtered.map(s => (
          <div key={s.id} style={{ display: "grid", gridTemplateColumns: "1.4fr 1.4fr 1.2fr 0.9fr 64px", alignItems: "center", padding: "13px 4px", borderBottom: "1px solid var(--neutral-100)" }}>
            <span style={{ fontSize: 13, fontWeight: 600, color: "var(--text-heading)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", paddingRight: 8 }}>{s.name}</span>
            <span style={{ fontSize: 12.5, color: "var(--text-body)" }}>{diasLabel(s.days)}</span>
            <span style={{ fontSize: 12.5, color: "var(--text-body)" }}>{horariosLabel(s.days)}</span>
            <span style={{ fontSize: 12.5, color: "var(--text-body)" }}>{mediaLabel(s.days)}</span>
            <div style={{ display: "flex", alignItems: "center", gap: 6, justifyContent: "flex-end" }}>
              <button onClick={() => setEditing(s)} title="Editar" style={iconBtn}><Pencil size={14} color="var(--text-subtle)" /></button>
              <button onClick={() => setDeleting(s)} title="Excluir" style={iconBtn}><Trash2 size={14} color="var(--text-subtle)" /></button>
            </div>
          </div>
        ))
      )}
      ...
      {deleting && (
        <div onClick={() => setDeleting(null)} style={overlay(400)}>
          <div onClick={e => e.stopPropagation()} style={{ background: "var(--surface-card)", borderRadius: 16, width: 380, padding: 22, boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text-heading)", marginBottom: 6 }}>Excluir horário</div>
            <p style={{ fontSize: 13, color: "var(--text-muted)", margin: "0 0 18px", lineHeight: 1.5 }}>Excluir <strong>{deleting.name}</strong>? Esta ação não pode ser desfeita.</p>
...
function ScheduleModal({ accent, editing, onClose, onSave }: {
  ...
  return (
    <div onClick={onClose} style={overlay(350)}>
      <div onClick={e => e.stopPropagation()} style={{ background: "var(--surface-card)", borderRadius: 16, width: 540, maxHeight: "85vh", display: "flex", flexDirection: "column", boxShadow: "0 24px 70px rgba(0,0,0,0.22)", overflow: "hidden" }}>
        <div style={{ padding: "18px 22px 0", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ fontSize: 18, fontWeight: 700, color: "var(--text-heading)" }}>{editing ? "Atualizar horário" : "Criar horário"}</div>
        ...
        <div style={{ padding: "16px 22px", overflowY: "auto" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {days.map((d, idx) => (
              <div key={d.day} style={{ background: d.active ? "var(--neutral-50)" : "transparent", border: "1px solid var(--neutral-100)", borderRadius: 10, padding: "10px 12px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <span style={{ fontSize: 13, fontWeight: 600, color: d.active ? "var(--text-heading)" : "var(--text-muted)", width: 110 }}>{DAY_FULL[d.day]}</span>
                  {!d.active && <span style={{ fontSize: 12, color: "var(--neutral-400)" }}>Fechado</span>}
                </div>
                {d.active && (
        ...
        <div style={{ padding: "14px 22px", borderTop: "1px solid var(--neutral-100)", display: "flex", justifyContent: "flex-end", gap: 10 }}>
...
function Toggle({ on, accent, onClick }: { on: boolean; accent: string; onClick: () => void }) {
  return (
    <button onClick={onClick} style={{ width: 38, height: 20, borderRadius: 10, background: on ? accent : "#D1D5DB", border: "none", cursor: "pointer", position: "relative", flexShrink: 0, transition: "background 0.2s" }}>
      <div style={{ width: 14, height: 14, borderRadius: "50%", background: "var(--surface-card)", position: "absolute", top: 3, left: on ? 21 : 3, transition: "left 0.2s", boxShadow: "0 1px 2px rgba(0,0,0,0.2)" }} />
    </button>
  );
}
...
const timeField: React.CSSProperties = { padding: "6px 8px", border: "1px solid var(--border-default)", borderRadius: 8, fontSize: 12.5, outline: "none", background: "var(--surface-card)" };
