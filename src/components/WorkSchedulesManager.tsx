  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 8, background: "hsl(var(--background))", border: "1px solid hsl(var(--border))", borderRadius: 10, padding: "8px 12px", marginBottom: 14 }}>
        ...
        <input placeholder="Pesquisar..." value={search} onChange={e => setSearch(e.target.value)} style={{ border: "none", outline: "none", background: "transparent", fontSize: 13, color: "hsl(var(--foreground))", flex: 1 }} />
        <span style={{ fontSize: 12, color: "hsl(var(--muted-foreground))" }}>{filtered.length} resultado{filtered.length === 1 ? "" : "s"}</span>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1.4fr 1.2fr 0.9fr 64px", padding: "6px 4px", borderBottom: "1px solid hsl(var(--border))", marginBottom: 4 }}>
      ...
      {loading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "48px 0", gap: 8 }}>
          <Clock size={32} color="hsl(var(--muted-foreground))" />
          <p style={{ fontSize: 13, color: "hsl(var(--muted-foreground))", margin: 0 }}>{items.length === 0 ? "Nenhum horário criado" : "Nenhum resultado"}</p>
          {items.length === 0 && <p style={{ fontSize: 12, color: "hsl(var(--muted-foreground))", margin: 0 }}>Clique em "Criar" para adicionar um horário</p>}
        </div>
      ) : (
        filtered.map(s => (
          <div key={s.id} style={{ display: "grid", gridTemplateColumns: "1.4fr 1.4fr 1.2fr 0.9fr 64px", alignItems: "center", padding: "13px 4px", borderBottom: "1px solid hsl(var(--border))" }}>
            <span style={{ fontSize: 13, fontWeight: 600, color: "hsl(var(--foreground))", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", paddingRight: 8 }}>{s.name}</span>
            <span style={{ fontSize: 12.5, color: "hsl(var(--foreground))" }}>{diasLabel(s.days)}</span>
            <span style={{ fontSize: 12.5, color: "hsl(var(--foreground))" }}>{horariosLabel(s.days)}</span>
            <span style={{ fontSize: 12.5, color: "hsl(var(--foreground))" }}>{mediaLabel(s.days)}</span>
            <div style={{ display: "flex", alignItems: "center", gap: 6, justifyContent: "flex-end" }}>
              <button onClick={() => setEditing(s)} title="Editar" style={iconBtn}><Pencil size={14} color="hsl(var(--muted-foreground))" /></button>
              <button onClick={() => setDeleting(s)} title="Excluir" style={iconBtn}><Trash2 size={14} color="hsl(var(--muted-foreground))" /></button>
            </div>
          </div>
        ))
      )}
      ...
      {deleting && (
        <div onClick={() => setDeleting(null)} style={overlay(400)}>
          <div onClick={e => e.stopPropagation()} style={{ background: "hsl(var(--card))", borderRadius: 14, width: 380, padding: 22, boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: "hsl(var(--foreground))", marginBottom: 6 }}>Excluir horário</div>
            <p style={{ fontSize: 13, color: "hsl(var(--muted-foreground))", margin: "0 0 18px", lineHeight: 1.5 }}>Excluir <strong>{deleting.name}</strong>? Esta ação não pode ser desfeita.</p>
...
function ScheduleModal({ accent, editing, onClose, onSave }: {
  ...
  return (
    <div onClick={onClose} style={overlay(350)}>
      <div onClick={e => e.stopPropagation()} style={{ background: "hsl(var(--card))", borderRadius: 14, width: 540, maxHeight: "85vh", display: "flex", flexDirection: "column", boxShadow: "0 24px 70px rgba(0,0,0,0.22)", overflow: "hidden" }}>
        <div style={{ padding: "18px 22px 0", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ fontSize: 16, fontWeight: 700, color: "hsl(var(--foreground))" }}>{editing ? "Atualizar horário" : "Criar horário"}</div>
        ...
        <div style={{ padding: "16px 22px", overflowY: "auto" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {days.map((d, idx) => (
              <div key={d.day} style={{ background: d.active ? "hsl(var(--muted))" : "transparent", border: "1px solid hsl(var(--border))", borderRadius: 10, padding: "10px 12px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <span style={{ fontSize: 13, fontWeight: 600, color: d.active ? "hsl(var(--foreground))" : "hsl(var(--muted-foreground))", width: 110 }}>{DAY_FULL[d.day]}</span>
                  {!d.active && <span style={{ fontSize: 12, color: "hsl(var(--muted-foreground))" }}>Fechado</span>}
                </div>
                {d.active && (
                  <div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 8, paddingLeft: 52 }}>
                    {d.intervals.map((iv, j) => (
                      <div key={j} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        <span style={{ fontSize: 12, color: "hsl(var(--muted-foreground))" }}>às</span>
                        ...
                        {d.intervals.length > 1 && (
                          <button onClick={() => patch(idx, x => ({ ...x, intervals: x.intervals.filter((_, k) => k !== j) }))} style={iconBtn}><Trash2 size={13} color="hsl(var(--destructive-soft-fg))" /></button>
                        )}
                      </div>
                    ))}
        ...
        <div style={{ padding: "14px 22px", borderTop: "1px solid hsl(var(--border))", display: "flex", justifyContent: "flex-end", gap: 10 }}>
...
function Toggle({ on, accent, onClick }: { on: boolean; accent: string; onClick: () => void }) {
  return (
    <button onClick={onClick} style={{ width: 38, height: 20, borderRadius: 10, background: on ? accent : "hsl(var(--muted))", border: "none", cursor: "pointer", position: "relative", flexShrink: 0, transition: "background 0.2s" }}>
      <div style={{ width: 14, height: 14, borderRadius: "50%", background: "hsl(var(--card))", position: "absolute", top: 3, left: on ? 21 : 3, transition: "left 0.2s", boxShadow: "0 1px 2px rgba(0,0,0,0.2)" }} />
    </button>
  );
}
...
const timeField: React.CSSProperties = { padding: "6px 8px", border: "1px solid hsl(var(--border))", borderRadius: 8, fontSize: 12.5, outline: "none", background: "hsl(var(--card))" };
