...
/**
 * Funil em camadas, com entrada animada e realce por etapa.
 *
 * ── O que foi mudado em relação ao componente de origem ──
 *
 * 1. `cn` vem de `@/lib/utils`. O original trazia a própria cópia (clsx +
 *    twMerge); duas fontes para a mesma função divergem no dia em que uma delas
 *    ganhar configuração.
 *
 * 2. As cores padrão. O original aponta para `--chart-1`, `--chart-grid`,
 *    `--chart-line-primary` e `--color-muted`, que NÃO existem neste projeto --
 *    aqui as variáveis guardam tripla HSL e são lidas como `hsl(var(--primary))`.
 *    Mantidas as do original, o gráfico sairia sem cor.
 *
 * 3. A escala. O original usa `max = data[0].value`, ou seja, assume que a
 *    primeira etapa é sempre a maior. Num funil de CRM uma etapa do meio pode
 *    ter mais negócios que a de entrada (negócio criado já dentro dela), e aí
 *    ela estouraria a caixa e a porcentagem passaria de 100%.
 */
...
export function FunnelChart({
  ...
  // Escala a partir do MAIOR valor, e não do primeiro como no original: uma
  // etapa do meio pode ter mais negócios que a de entrada (leads criados já
  // dentro dela), e nesse caso a do meio estouraria a caixa e a porcentagem
  // passaria de 100%.
  ...
  const gridLineColor = gridCfg.lineColor ?? "hsl(var(--card-border))";
