...
export default function AgentUserView({ role = "SDR", userName = "Carlos Andrade" }: Props) {
  ...
  return (
    <div className="p-6 max-w-[1400px] mx-auto">
      {/* Header */}
      <div className="mb-5">
        <h1 className="text-[20px] font-bold text-[color:var(--text-heading)] leading-tight">Meu Agente</h1>
      ...
      {!bannerDismissed && (
        ...
        >
          <Info size={18} color="var(--text-link)" className="shrink-0 mt-0.5" />
          ...
          <button
            ...
            className="text-[12px] font-semibold text-[color:var(--text-link)] hover:text-[#1E3A5F] transition-colors px-2 py-0.5 rounded hover:bg-white/50 shrink-0"
          >
            Entendi
          </button>
          <button onClick={dismissBanner} className="text-[color:var(--text-link)] hover:text-[#1E3A5F] shrink-0 transition-colors">
            <X size={15} />
          </button>
        </div>
      )}
      ...
      <div className="grid grid-cols-[340px_1fr] gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-card border border-[color:var(--border-default)] rounded-2xl shadow-elev-1 p-5">
            <div className="flex items-center gap-3">
              ...
              >
                {role === "SDR" || role === "Closer"
                  ? <UserIcon size={22} color="var(--accent-800)" />
                  : <Sparkles size={22} color="var(--accent-800)" />
                }
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-[14px] font-bold text-[color:var(--text-heading)] truncate">
            ...
            <p className="text-[12px] text-[color:var(--text-muted)] mt-3 leading-relaxed">
          ...
          <div
            ...
            style={{ background: "var(--accent-50)", border: "1px solid #0F6E56" }}
          >
            {/* Header */}
            <div className="flex items-center gap-2 mb-3">
              <Clock size={16} color="var(--accent-800)" />
              <div className="flex-1">
                <div className="text-[13px] font-semibold text-[color:var(--accent-800)]">Produtividade de Hoje</div>
                <div className="text-[12px] text-[color:var(--accent-800)]/70">
            ...
            {/* Tempo ativo */}
            <div className="text-[32px] font-bold text-[color:var(--accent-800)] leading-none">6h 12min</div>
            <div className="text-[12px] text-[color:var(--accent-800)]/80 mt-1">Tempo ativo no Rezult</div>
            <div className="mt-2">
              <Progress value={77} className="h-1.5 bg-white/60" />
              <div className="text-[12px] text-[color:var(--accent-800)]/70 mt-1">Meta diária 8h — 77% atingido</div>
            </div>
            ...
            <div className="mt-4 bg-card rounded-lg p-3.5 text-center">
              ...
              <div className="text-[12px] text-[color:var(--text-muted)] mt-1">Score do agente hoje</div>
        ...
        <div>
          <Tabs defaultValue="hoje">
            <TabsList className="bg-card border border-[color:var(--border-default)] p-1 rounded-lg">
              {[
                ...
                <TabsTrigger
                  key={t.v}
                  value={t.v}
                  className="data-[state=active]:bg-[color:var(--accent-100)] data-[state=active]:text-[color:var(--accent-800)] data-[state=active]:shadow-none text-[13px]"
                >
                  {t.l}
                </TabsTrigger>
              ))}
            ...
            <TabsContent value="hoje" className="mt-4 space-y-4">
              ...
              <div
                className="rounded-xl p-5"
                style={{ background: "var(--neutral-25)", border: "1px solid var(--border-default)", borderRadius: 12 }}
              >
                {/* Header do card */}
                <div className="flex items-start gap-2 mb-5">
                  <Sparkles size={16} color="var(--accent-700)" className="mt-0.5 shrink-0" />
                  <div className="flex-1">
                    <div className="text-[14px] font-semibold text-[color:var(--text-heading)]">
                      Relatório do Agent {role}
                    </div>
                    <div className="text-[12px] text-[color:var(--text-muted)] mt-0.5">Gerado hoje às 18h30</div>
                  </div>
                  <Badge
                ...
                {REPORT_SECTIONS.map((p, i) => (
                  <div key={p.label} className={i < REPORT_SECTIONS.length - 1 ? "mb-5" : ""}>
                    ...
                    <p className="text-[13px] text-[color:var(--text-body)] leading-relaxed">{p.text}</p>
                  </div>
                ))}
              </div>
                  ...
                  {ACTIVITIES.map((a, i) => {
                    ...
                    return (
                      <div key={i} className="flex items-start gap-3 relative">
                        ...
                        <div className="text-[12px] text-[color:var(--text-muted)] font-mono w-10 shrink-0 pt-1">
                          {a.time}
                        </div>
                        <div
                        ...
                        >
                          <Ico size={13} color="var(--accent-700)" />
                        </div>
                        <div className="flex-1 text-[13px] text-[color:var(--text-heading)] pt-1">
                          {a.text}
                          {a.lead && (
                            <>
                              {": "}
                              <button className="text-[color:var(--text-link)] hover:underline">
                                {a.lead}
                              </button>
                            </>
                          )}
            ...
            <TabsContent value="semana" className="mt-4 space-y-4">
              <div className="bg-card border border-[color:var(--border-default)] rounded-2xl shadow-elev-1 p-5">
                ...
                {/* Gráfico de barras */}
                <div className="flex items-end gap-4 h-[160px] pb-6 border-b border-[color:var(--border-default)]">
                  {WEEK.map((d, i) => {
                    const isToday = i === WEEK.length - 1;
                    const hPct    = (d.score / 10) * 100;
                    return (
                      <div key={d.day} className="flex-1 flex flex-col items-center gap-1.5">
                        <div className="text-[12px] font-semibold" style={{ color: isToday ? "var(--accent-800)" : "var(--text-heading)" }}>
                          {d.score}
                        </div>
                        <div
                          ...
                          style={{
                            height: `${hPct}%`,
                            background: isToday ? "#0F6E56" : "var(--accent-50)",
                          }}
                        />
                        <div className="text-[12px]" style={{ color: isToday ? "var(--accent-800)" : "var(--text-muted)" }}>
                          {d.day}
                        </div>
                      </div>
                    );
                ...
                <table className="w-full mt-4 text-[12px]">
                  ...
                  <tbody>
                    {WEEK.map((d, i) => {
                      const isToday = i === WEEK.length - 1;
                      return (
                        <tr key={d.day} className="border-t border-[color:var(--border-default)]">
                          <td className="py-2 font-semibold" style={{ color: isToday ? "var(--accent-800)" : "var(--text-heading)" }}>{d.day}</td>
                          <td className="py-2 text-[color:var(--text-muted)]">{d.time}</td>
                          <td className="py-2 text-[color:var(--text-muted)]">{d.leads}</td>
                ...
                {[
                  ...
                  >
                    ...
                    <div className="mt-2 flex items-baseline gap-1">
                      <span className="text-[24px] font-bold text-[color:var(--text-heading)]">{c.value}</span>
                      {c.sub && <span className="text-[12px] text-[color:var(--text-muted)]">{c.sub}</span>}
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="bg-card border border-[color:var(--border-default)] rounded-2xl shadow-elev-1 p-5">
                ...
                <div className="grid grid-cols-7 gap-1.5 mb-1">
                  {["D", "S", "T", "Q", "Q", "S", "S"].map((d, i) => (
                    <div key={i} className="text-center text-[12px] text-[color:var(--text-muted)] font-medium">{d}</div>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-7 gap-1.5">
                  {monthDays.map(d => (
                    <div
                      key={d.day}
                      className="aspect-square rounded-md flex flex-col items-center justify-center text-[12px] font-medium cursor-default select-none"
                      style={{
                        background: d.score === null ? "var(--neutral-50)" : scoreColor(d.score),
                        color: d.score === null ? "var(--neutral-400)" : "#FFFFFF",
                      }}
                      title={d.score !== null ? `Score ${d.score}` : "Sem atividade"}
                    >
                      <span>{d.day}</span>
                      {d.score !== null && (
                        <span className="text-[12px] opacity-90 leading-none mt-0.5">{d.score}</span>
                      )}
                    </div>
                  ))}
                </div>
                ...
                {/* Legenda */}
                <div className="flex items-center gap-4 mt-4 text-[12px] text-[color:var(--text-muted)] flex-wrap">
                  {[
                    { c: "#0F6E56", l: "9+" },
                    { c: "#34C77B", l: "7–8.9" },
                    { c: "#F59E0B", l: "5–6.9" },
                    ...
                    <div key={le.l} className="flex items-center gap-1.5">
                      <span
                        ...
                        style={{ background: le.c, border: le.c === "var(--neutral-100)" ? "1px solid var(--border-default)" : "none" }}
                      />
                      {le.l}
                    </div>
                  ))}
              ...
              <div className="bg-card border border-[color:var(--border-default)] rounded-2xl shadow-elev-1 p-5">
                ...
                <div className="space-y-0">
                  {mockReports.map((r, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-3 py-2.5 border-b border-[color:var(--neutral-100)] last:border-0"
                    >
                      <div className="text-[12px] text-[color:var(--text-muted)] w-24 shrink-0">
                        {r.date.toLocaleDateString("pt-BR")}
                      </div>
                      <div className="text-[12px] text-[color:var(--text-muted)] flex-1">
                        {r.time} ativo
                      </div>
                      <div
