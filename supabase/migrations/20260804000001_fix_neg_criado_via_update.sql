-- Corrige leads_automation_trigger_fn(): "neg_criado" (Negócio criado) só
-- disparava no INSERT quando o lead já nascia com pipeline_id preenchido.
-- Fluxos que criam o Lead solto primeiro e só depois anexam o negócio via
-- UPDATE (ex.: automações com ações separadas "Criar lead" + "Criar
-- negócio", padrão comum no construtor) nunca disparavam "neg_criado" --
-- o banco via isso como "negócio movido de etapa" (neg_movido), já que
-- column_id/pipeline_id mudaram numa linha já existente, não numa inserção.
-- Confirmado no caso real: Consultório Samantha, automação "Mensagem de
-- boas-vindas" (trigger "Negócio criado") nunca rodava pros leads vindos
-- da automação "Formulário instantâneo sheets", que usa esse padrão de
-- 2 passos.
--
-- Fix: no UPDATE, se o lead não tinha pipeline_id antes e passou a ter,
-- também disparamos "neg_criado" -- além do "neg_movido" que já dispara
-- hoje pra esse mesmo caso (não removido: quem já escuta neg_movido pra
-- capturar "primeira etapa" continua funcionando).
...
BEGIN
  ...
  -- Negócio criado depois do lead (pipeline_id chegou via UPDATE, não no INSERT)
  IF OLD.pipeline_id IS NULL AND NEW.pipeline_id IS NOT NULL THEN
    PERFORM public.dispatch_automation_event(
      'neg_criado', v_company_id, NEW.id,
      jsonb_build_object('pipeline_id', NEW.pipeline_id, 'new_column_id', NEW.column_id)
    );
  ...
  IF NEW.column_id IS DISTINCT FROM OLD.column_id AND NEW.pipeline_id IS NOT NULL THEN
    PERFORM public.dispatch_automation_event(
  ...
  IF NEW.status IS DISTINCT FROM OLD.status AND NEW.pipeline_id IS NOT NULL THEN
