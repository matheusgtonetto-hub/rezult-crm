-- Substitui a de 20260908203228. Única mudança: o ramo `responsibles` passa a
-- entender o valor sentinela '__no_responsible__'.
--
-- ── Por que uma função de busca precisa saber disso ──
--
-- O kanban tem dois recortes por responsável, e eles são de naturezas
-- diferentes:
--
--   advFilter.responsibles  o filtro que a pessoa escolheu no painel
--   podeVer()               quem ela tem DIREITO de enxergar
--
-- O segundo vem de `myPerms.viewOwnDealsOnly` (só os próprios negócios) e do
-- seletor "Visualizando como" do admin, que aceita a opção "Sem responsável".
-- No cliente essa opção é o texto '__no_responsible__', e significa o oposto de
-- um nome: casa com quem NÃO tem responsável nenhum.
--
-- Enquanto o board carregava tudo, `podeVer` rodava em memória depois da busca.
-- Com paginação isso deixa de funcionar: filtrar a página de 50 depois de
-- recebida daria uma página de tamanho imprevisível e um total errado no
-- cabeçalho, porque a contagem no banco não saberia do recorte. Todo critério
-- que decide o que aparece tem que estar na mesma consulta que conta.
--
-- ── Por que não virou uma chave nova ──
--
-- O sentinela entra no mesmo array `responsibles` porque a semântica é a mesma
-- pergunta ("de quem são estes negócios?"), só que com uma resposta a mais.
-- Uma chave separada obrigaria quem chama a decidir em qual das duas pôr cada
-- item, e as duas teriam que ser combinadas com OR do mesmo jeito.
--
-- Cuidado ao ler: o array vazio de responsáveis é o CASO de match aqui, não a
-- ausência de filtro. Por isso o `cardinality(...) = 0` está dentro do OR e não
-- é uma guarda.
--
-- O resto do corpo é idêntico ao de 20260908203228.
