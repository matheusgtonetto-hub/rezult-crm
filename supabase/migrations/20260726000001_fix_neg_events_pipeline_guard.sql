-- Corrige leads_automation_trigger_fn(): eventos "neg_*" (negócio criado/movido/
-- ganho/perdido/restaurado) disparavam incondicionalmente, mesmo para um Lead sem
-- negócio (pipeline_id null, possível desde 20260725000005_leads_pipeline_optional.sql).
-- Isso fazia uma automação "Negócio criado" sem filtro de pipeline disparar para
-- todo "+ Lead" criado no Multiatendimento, que não é um negócio.
-- lead_criado, atend_*, tag_*, campo_alterado seguem incondicionais (corretos:
-- valem tanto pra Lead quanto pra Negócio).
