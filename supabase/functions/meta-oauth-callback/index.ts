...
serve(async (req) => {
  ...
  const { code, redirect_uri, provider } = await req.json() as {
    code: string;
    redirect_uri: string;
  ...
  if (!tokenData.access_token) {
    ...
    return json({ error: "falha ao trocar código OAuth", detail: tokenData.error?.message }, 400);
  }
  ...
  // Usa a primeira página elegível (pode ser expandido para seleção futura)
